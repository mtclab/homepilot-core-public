# agent-host (Python — legacy)

> **The shipped agent is now the Go rewrite in [`agent/go/`](go/README.md)** — a
> static binary (no glibc/python on hosts), amd64 + arm64, protocol-compatible.
> This Python package is kept as reference and for its unit tests; new
> deployments use the Go binary.
...
```bash
sudo cp agent-host.service /etc/systemd/system/
