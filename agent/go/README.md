...
```bash
cd agent/go
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o agent-host-amd64 .
CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags="-s -w" -o agent-host-arm64 .
go vet ./... && go test ./...
```
...
```bash
sudo install -m 0755 agent-host-amd64 /usr/local/bin/agent-host
# unit (agent/agent-host.service) sets the Environment= lines, then:
sudo systemctl enable --now agent-host
```

Replace a running binary atomically: `scp` to `/usr/local/bin/agent-host.new`,
`mv -f` over `/usr/local/bin/agent-host`, `systemctl restart agent-host`.
