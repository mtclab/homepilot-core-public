...
```env
HP_PROXMOX_HOST=pve.example.local   # Proxmox VE host
HP_PROXMOX_VERIFY_SSL=false
```

Or use the vault (recommended) — `hp init` sets up all secrets automatically.
...
Systemd unit file is included at `agent/hp-agent.service` (calls `/usr/local/bin/hp-agent`).
