 ...
 *
 * Run it on the staging box, never on the workspace, with the control plane's
 * address in BASE and its token on stdin:
 *   printf '%s' "$TOKEN" | ssh <staging-host> \
 *     'BASE=http://<control-plane>:8000 node ui-blind-spots.mjs'
 *
 * No address or account name is hardcoded here: this file ships to the public
 * mirror, and an estate address in a committed script is a leak whether or not
 * the scrub happens to know the extension (it did not, on 2026-08-30).
 */
...
if (!BASE) {
	console.error('FAIL: set BASE to the control plane, e.g. http://host:8000');
	process.exit(2);
}
