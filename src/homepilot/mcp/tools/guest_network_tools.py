...
TOOL_DEFINITIONS: list[dict[str, Any]] = [
    {
        ...
        "description": (
            ...
            "'enforcement' says whether the guest fence is actually ENFORCED: "
            "the per-VM rule set is inert until the PVE datacenter firewall "
            "master switch is on (survey 'datacenter_firewall_enabled' / "
            "'fence_enforced'), so 'configured but not enforced' is a visible "
            "state; and under the legacy iptables stack the vnet forward rules "
            "are stored but NOT applied. Applying the artifact enables the "
            "datacenter firewall the only safe way (policy_in=ACCEPT, no "
            "lockout). To CHANGE any of it, "
