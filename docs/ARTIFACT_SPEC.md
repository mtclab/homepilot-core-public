...
2. Build a temporary inventory file pointing at that host, with `ansible_ssh_common_args="-o ProxyJump=jump-server-internal"` so SSH routes through the jump server.
...
2. SSH to target via jump server.
